...
基于 **LangGraph** 构建的企业级知识库智能问答系统。面向企业内部知识管理场景，支持**类型自适应分块**、**稠密 + 稀疏混合检索 + 重排序**、**Agentic 决策流**、**SSE 流式输出**，并提供 **LLM-as-Judge 评测框架**与**分块策略消融实验**。
...
- **类型自适应分块**：6 种文档类型 × 6 种分块策略（条款型/问答型/步骤型/接口型/章节型/表格型），按文档结构选择最优切分方式
- **混合检索**：稠密向量（语义）+ 稀疏向量（关键词）+ RRF 融合 + CrossEncoder 重排序
- **父子块策略**：子块精确定位 + 父块为 LLM 提供丰富上下文
- **Agentic 决策**：意图分类自动路由（闲聊直答 / 知识问答），检索质量不足时 LLM 改写查询重检（最多 3 轮）
...
| 文档类型 | 结构特征 | 分块策略 |
|---------|---------|---------|
| 规章制度 | 「第X条」条款 | `_split_by_articles` 按条切分，条款不跨块 |
| FAQ | Q/A 问答对 | `_split_faq` 检测问答对，整体不切断 |
| 操作流程SOP | 「步骤1/2/3」编号 | `_split_by_steps` 按步骤切分，保持序列 |
...
输出 5 条策略在 Recall@k / Precision@k / MRR@k / nDCG@k / HitRate@k 上的对比，含 Bootstrap 95% CI 与分层统计。
